...
def update_clustroids(points: list[str], clusters: list[int]):
    ...
    for c_points in cluster_points_list:
        ...
        for index, row in enumerate(lev_distance):
            ...
            if masked_row[max_index] < min_max_distance:
                new_clustroid_index = index
                min_max_distance = masked_row[max_index]
...
def print_kmeans_results(num_clusters: int, clusters: list[int], points: list[str], time_per_repetition: list[float], steps_per_repetition: list[int]):
    cluster_dict = {}
...
def kmeans(num_clusters: int, repetitions: int, points: list[str]):
    ...
    print_kmeans_results(num_clusters, clusters, points, time_per_repetition, steps_per_repetition)
    return
...
def main():
    ...
    kmeans(NUM_CLUSTERS, REPETITIONS, titles)
